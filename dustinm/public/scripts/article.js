...
let Article = ((module)=> {
  ...
  */
  ...
  Article.numWordsAll = () => {
    return Article.all.map((article)=> {return article.body}).reduce((wordcount,body)=> {
      return wordcount += body.split(' ').length;
    },0)
  };
  ...
  Article.allAuthors = () => {
    ...
    return Article.all.map((article)=> {return article.author}).reduce((shortlist, author)=> {
      if(!shortlist.includes(author)) shortlist.push(author);
      return shortlist;
    },[])
  };
